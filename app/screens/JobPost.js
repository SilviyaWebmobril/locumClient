...
class JobPost extends Component {
      ...
      NetInfo.isConnected.fetch().then(isConnected => {
        if (!isConnected) {
          ...
          try{
            ...
            formData.append('from_time',this.state.from_time);
            formData.append('to_time',this.state.to_time);
  ...
  showFromTimePicker = () => {
    this.setState({ from_time: true })
  }

  hideFromTimePicker = () => {
    this.setState({ from_time: false });
  }

  showToimePicker = () => {
    this.setState({ to_time: true })
  }

  hideToTimePicker = () => {
    this.setState({ to_time: false });
  }
      ...
      <View style={{ flex: 1 }}>
        ...
        <View style={styles.container}>
          ...
          <View style={{ flexDirection: 'row', justifyContent: 'space-between', alignItems: 'center', width: '100%', height: '09%', backgroundColor: '#4C74E6' }}>
            ...
            <View>
              <Text style={{fontFamily:"Roboto-Light",  fontSize: 20, fontWeight: 'bold', color: "white", paddingRight: 25 }}>Post Job</Text>
            </View>
